@extends('app')
@section('title')
    {{ __('app.create_new') }} | {{ __('app.app_name') }}
@endsection
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 md:mb-8 gap-4">
        <div class="min-w-0">
            <nav class="flex gap-2 text-label-sm font-label-sm text-on-surface-variant mb-2 overflow-x-auto overflow-hidden">
                ...
                <span class="text-primary-fixed whitespace-nowrap">{{ __('app.create_new') }}</span>
            </nav>
            <h1 class="font-display-lg text-display-lg-mobile md:text-display-lg text-on-surface tracking-tight break-words">{{ __('app.nexus_content_editor') }}</h1>
        </div>
    </div>
                                        ...
                                        @foreach (range(0, 3) as $j)
                                            <div class="flex items-center gap-2 mb-2">
                                                ...
                                                <input type="radio" name="quizzes[{{ $i }}][correct_answer]"
                                                    value="{{ $j }}" @checked(old('correct_answer', (string) $j)) />
                                            </div>
                                        @endforeach
                                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                            ...
                            <select name="status"
                                ...
                                <option value="draft" @selected(old('status', 'draft') === 'draft')>{{ __('app.draft') }}</option>
                                <option value="published" @selected(old('status') === 'published')>{{ __('app.published') }}</option>
                            </select>
                        </div>
                        <div class="flex items-center justify-between">
