@extends('app')
@section('title')
    {{ __('app.create_submission') }} | {{ __('app.app_name') }}
@endsection
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 md:mb-8 gap-4">
        <div class="min-w-0">
            <nav class="flex gap-2 text-label-sm font-label-sm text-on-surface-variant mb-2 overflow-x-auto overflow-hidden">
                ...
                <span class="text-primary-fixed whitespace-nowrap">{{ __('app.create_submission') }}</span>
            </nav>
            <h1 class="font-display-lg text-display-lg-mobile md:text-display-lg text-on-surface tracking-tight break-words">{{ __('app.create_submission') }}</h1>
        </div>
    </div>

    <form method="POST" action="{{ route('user.content.store') }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="grid grid-cols-12 gap-gutter">
            <div class="col-span-12 xl:col-span-8 space-y-6 md:space-y-8">
                    ...
                    <div class="space-y-6 md:space-y-8">
                        ...
                        <div class="relative">
                            ...
                            <textarea name="description" rows="3"
                                ...
                                placeholder="{{ __('app.description_placeholder') }}">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="text-error text-label-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                                    ...
                                    @foreach (config('sdg.categories') as $catKey => $category)
                                        <optgroup label="{{ __("app.category_{$catKey}") }}">
                                            @foreach ($category['sdgs'] as $num)
                                                <option value="SDG {{ $num }}" @selected(old('sdg_category') === 'SDG '.$num)>{{ __("app.sdg_{$num}") }}</option>
                                            @endforeach
                                        </optgroup>
                                    @endforeach
                                </select>
                ...
                <section class="glass-card rounded-2xl overflow-hidden flex flex-col h-[300px] md:h-[500px]">
                    ...
                    <div class="flex-1 p-4 md:p-8 overflow-y-auto" id="editor-wrapper">
                        <div name="body-editor" contenteditable="true"
                            ...
                            id="body-editor">{!! old('body') ? nl2br(e(old('body'))) : '<p><br></p>' !!}</div>
                ...
                <section id="flashcards-section" class="glass-card rounded-2xl p-4 md:p-8">
                    ...
                    <p id="no-flashcards-msg" class="text-label-sm text-on-surface-variant/60 text-center py-4 {{ old('flashcards') ? 'hidden' : '' }}">{{ __('app.no_flashcards') }}</p>
                </section>
                ...
                <section id="quizzes-section" class="glass-card rounded-2xl p-4 md:p-8">
                                        ...
                                        @foreach (range(0, 3) as $j)
                                            <div class="flex items-center gap-2 mb-2">
                                                ...
                                                <input type="radio" name="quizzes[{{ $i }}][correct_answer]"
                                                    value="{{ $j }}" @checked(old("quizzes.{$i}.correct_answer") == $j) />
                                            </div>
                                        @endforeach
                                    </div>
                    ...
                    <p id="no-quizzes-msg" class="text-label-sm text-on-surface-variant/60 text-center py-4 {{ old('quizzes') ? 'hidden' : '' }}">{{ __('app.no_quizzes') }}</p>
                </section>
            </div>
            ...
            <div class="col-span-12 xl:col-span-4 space-y-6 md:space-y-8">
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <label class="py-2 text-[10px] font-bold border border-outline-variant/30 rounded hover:border-primary-container transition-all text-center cursor-pointer has-[:checked]:border-primary-container has-[:checked]:bg-primary-container/10 has-[:checked]:text-primary-container">
                                    <input type="radio" name="difficulty" value="basic"
                                        class="sr-only" @checked(old('difficulty') === 'basic') />
                                    {{ __('app.basic') }}
                                </label>
                                <label class="py-2 text-[10px] font-bold border border-outline-variant/30 rounded hover:border-primary-container transition-all text-center cursor-pointer has-[:checked]:border-primary-container has-[:checked]:bg-primary-container/10 has-[:checked]:text-primary-container">
                                    <input type="radio" name="difficulty" value="core"
                                        class="sr-only" @checked(old('difficulty', 'core') === 'core') />
                                    {{ __('app.core') }}
                                </label>
                                <label class="py-2 text-[10px] font-bold border border-outline-variant/30 rounded hover:border-primary-container transition-all text-center cursor-pointer has-[:checked]:border-primary-container has-[:checked]:bg-primary-container/10 has-[:checked]:text-primary-container">
                                    <input type="radio" name="difficulty" value="expert"
                                        class="sr-only" @checked(old('difficulty') === 'expert') />
                                    {{ __('app.expert') }}
                                </label>
                            </div>
                        </div>
                ...
                <section class="glass-card-static rounded-2xl p-4 md:p-6 bg-gradient-to-br from-surface-container-high/50 to-primary-container/5 border border-primary-container/20">
                    <div class="flex items-center gap-2 mb-4">
                        <span class="material-symbols-outlined text-primary-container" style="font-variation-settings: 'FILL' 1;">info</span>
                        <h4 class="font-headline-md text-headline-md text-primary-fixed">{{ __('app.submit_content') }}</h4>
                    </div>
                    <div class="space-y-4">
...
<script>
    ...
    let fcIndex = {{ old('flashcards') ? count(old('flashcards')) : 0 }};
    let qzIndex = {{ old('quizzes') ? count(old('quizzes')) : 0 }};
